class RakutenPayEmailProcessor {
    ...
    getPurchaseRecords(): PurchaseRecord[] {
        ...
        eachMessage(GMAIL_SEARCH_CRITERIA, (message: GoogleAppsScript.Gmail.GmailMessage) => {
            try {
                const purchaseRecord = this.extractPurchaseRecord(message);
    ...
    private extractPurchaseRecord(message: GoogleAppsScript.Gmail.GmailMessage): PurchaseRecord {
        const normalizedText = message.getBody().replace(/\r\n|\r|\n/g, ' ');
        return {
            date: this.getDate(normalizedText),
            shopName: this.getShopName(normalizedText),
            amount: this.getTotalSettlement(normalizedText)
        };
    }

    private getDate(normalizedText: string): string {
        try {
            ...
            const value = this.extractValueFromHtmlText(dateMatch[0]);
    ...
    private getShopName(normalizedText: string): string {
        try {
            ...
            return this.extractValueFromHtmlText(shopMatch[0]);
        } catch (error) {
    ...
    private getTotalSettlement(normalizedText: string): string {
        try {
            ...
            let value = this.extractValueFromHtmlText(amountMatch[0]);
    ...
    private extractValueFromHtmlText(htmlText: string): string {
        try {
